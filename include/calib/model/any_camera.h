...
class CALIB_EXPORT AnyCamera {
  public:
    ...
    AnyCamera(M&& cam) : cam_(std::move(cam)) {
        traits_ = {static_cast<int>(CameraTraits<M>::param_count), CameraTraits<M>::idx_fx,
                   CameraTraits<M>::idx_fy, CameraTraits<M>::idx_skew};
    ...
    bool has_value() const noexcept { return !std::holds_alternative<std::monostate>(cam_); }
    ...
    template <typename T>
    Eigen::Matrix<T, 2, 1> project(const Eigen::Matrix<T, 3, 1>& X, const T* intr) const {
        return std::visit(
    ...
    template <typename T>
    Eigen::Matrix<T, 3, 1> unproject(const Eigen::Matrix<T, 2, 1>& x, const T* intr) const {
        return std::visit(
  ...
  private:
    using CameraVariant = std::variant<std::monostate, Camera<BrownConradyd>,
