...
struct ExtrinsicBlocks final : public ProblemParamBlocks {
    ...
    void populate_result(ExtrinsicOptimizationResult& result) const {
        ...
        for (size_t cam_idx = 0; cam_idx < num_cams; ++cam_idx) {
            ...
            cam.params() = Eigen::Map<const Eigen::VectorXd>(intrinsics[cam_idx].data(), intr_size);
...
static ceres::Problem build_problem(const std::vector<MulticamPlanarView>& views,
                                    const ExtrinsicOptions& options, ExtrinsicBlocks& blocks) {
    ...
    for (size_t view_idx = 0; view_idx < views.size(); ++view_idx) {
        ...
        for (size_t cam_idx = 0; cam_idx < blocks.cams.size(); ++cam_idx) {
            ...
            problem.AddResidualBlock(
                ExtrinsicResidual::create(multicam_view[cam_idx], blocks.cams[cam_idx]), loss,
                blocks.cam_quat_ref[cam_idx].data(), blocks.cam_tran_ref[cam_idx].data(),
                blocks.ref_quat_tgt[view_idx].data(), blocks.ref_tran_tgt[view_idx].data(),
                blocks.intrinsics[cam_idx].data());
        }
    }
