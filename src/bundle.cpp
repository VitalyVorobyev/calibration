...
struct BundleBlocks final : public ProblemParamBlocks {
    ...
    void populate_results(BundleResult& result) const {
        ...
        for (size_t cam_idx = 0; cam_idx < num_cams; ++cam_idx) {
            ...
            cam.params() = Eigen::Map<const Eigen::VectorXd>(intr[cam_idx].data(), intr_size);
...
static ceres::Problem build_problem(const std::vector<BundleObservation>& observations,
                                    const BundleOptions& opts, BundleBlocks& blocks) {
    ...
    for (const auto& obs : observations) {
        ...
        problem.AddResidualBlock(
            BundleReprojResidual::create(obs.view, obs.b_se3_g, blocks.cams[cam_idx]), loss,
    ...
    for (auto& q : blocks.g_q_c) problem.SetManifold(q.data(), new ceres::QuaternionManifold());
    ...
    if (!opts.optimize_hand_eye) {
        for (auto& quat_block : blocks.g_q_c) problem.SetParameterBlockConstant(quat_block.data());
        for (auto& tran_block : blocks.g_t_c) problem.SetParameterBlockConstant(tran_block.data());
    }
    if (!opts.optimize_intrinsics) {
        for (auto& intr_block : blocks.intr) problem.SetParameterBlockConstant(intr_block.data());
    } else {
        for (size_t cam_idx = 0; cam_idx < blocks.intr.size(); ++cam_idx) {
            ...
            if (!opts.optimize_skew) {
                problem.SetManifold(intr_block.data(),
                                    new ceres::SubsetManifold(blocks.intr_size, {traits.idx_skew}));
            }
        }
    }
    return problem;
