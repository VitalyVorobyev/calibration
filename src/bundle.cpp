...
static ScheimpflugBundleBlocks initialize_blocks_scheimpflug(
    ...
    const Eigen::Affine3d& b_T_t)
{
    ScheimpflugBundleBlocks blocks; const size_t ncam = c_T_r.size();
    populate_quat_tran(g_T_r, blocks.g_q_r, blocks.g_t_r);
    populate_quat_tran(b_T_t, blocks.b_q_t, blocks.b_t_t);
    blocks.c_q_r.resize(ncam); blocks.c_t_r.resize(ncam); blocks.intr.resize(ncam);
    for (size_t i=0;i<ncam;++i){
        populate_quat_tran(c_T_r[i], blocks.c_q_r[i], blocks.c_t_r[i]);
        const auto& cam = initial_cameras[i];
        std::array<double,11> intr = {
            cam.camera.K.fx, cam.camera.K.fy, cam.camera.K.cx, cam.camera.K.cy,
            0,0,0,0,0,
            cam.tau_x, cam.tau_y};
        for (int d=0; d<std::min<int>(5, cam.camera.distortion.forward.size()); ++d){
            intr[4+d] = cam.camera.distortion.forward(d);
        }
        blocks.intr[i] = intr;
    }
    return blocks;
...
static void build_problem_scheimpflug(const std::vector<BundleObservation>& observations,
                                      const BundleOptions& opts,
                                      ScheimpflugBundleBlocks& blocks,
                                      ceres::Problem& p) {
    for (const auto& obs : observations){
        const size_t cam = obs.camera_index;
        auto* cost = HandEyeScheimpflugReprojResidual::create(obs.view, obs.b_T_g);
        p.AddResidualBlock(cost, new ceres::HuberLoss(1.0),
                           blocks.b_q_t.data(), blocks.b_t_t.data(),
                           blocks.g_q_r.data(), blocks.g_t_r.data(),
                           blocks.c_q_r[cam].data(), blocks.c_t_r[cam].data(),
...
static void recover_parameters(const ScheimpflugBundleBlocks& blocks,
    ...
    result.g_T_r = restore_pose(blocks.g_q_r, blocks.g_t_r);
    const size_t num_cams = blocks.intr.size();
    result.c_T_r.resize(num_cams); result.cameras.resize(num_cams);
    for (size_t c=0;c<num_cams;++c){
        result.c_T_r[c] = restore_pose(blocks.c_q_r[c], blocks.c_t_r[c]);
        const auto& i = blocks.intr[c];
        CameraMatrix K{i[0],i[1],i[2],i[3]};
        Eigen::VectorXd dist(5); dist<<i[4],i[5],i[6],i[7],i[8];
        Camera cam(K, dist);
        result.cameras[c] = ScheimpflugCamera(cam, i[9], i[10]);
    }
}
    ...
    ScheimpflugBundleResult& result)
{
    ...
    for (const auto& obs : observations){
        ...
        auto c_T_t = get_camera_T_target(
            result.b_T_t, result.g_T_r, result.c_T_r[cam_idx], obs.b_T_g);
        double u_hat,v_hat; std::array<double,11> intr = {
            sc.camera.K.fx, sc.camera.K.fy, sc.camera.K.cx, sc.camera.K.cy,
            sc.camera.distortion.forward.size()>0?sc.camera.distortion.forward(0):0.0,
            sc.camera.distortion.forward.size()>1?sc.camera.distortion.forward(1):0.0,
            sc.camera.distortion.forward.size()>2?sc.camera.distortion.forward(2):0.0,
            sc.camera.distortion.forward.size()>3?sc.camera.distortion.forward(3):0.0,
            sc.camera.distortion.forward.size()>4?sc.camera.distortion.forward(4):0.0,
            sc.tau_x, sc.tau_y};
        Eigen::Vector3d P;
        for (const auto& ob : obs.view){
            P << ob.object_xy.x(), ob.object_xy.y(), 0;
            P = c_T_t * P;
            project_scheimpflug_with_intrinsics(P(0), P(1), P(2), intr.data(), true, u_hat, v_hat);
            double du = u_hat - ob.image_uv.x();
            double dv = v_hat - ob.image_uv.y();
            ssr += du*du + dv*dv; total+=2;
        }
    }
    return total? std::sqrt(ssr/total) : 0.0;
}
...
static Eigen::MatrixXd compute_covariance(ceres::Problem& p,
    ...
    if (!cov.Compute(blocks_list, &p)) return Eigen::MatrixXd();
    std::vector<int> sizes(blocks_list.size());
    for (size_t i=0;i<blocks_list.size();++i) sizes[i]=p.ParameterBlockSize(blocks_list[i]);
    int dim=0; for (int s:sizes) dim+=s;
    Eigen::MatrixXd cov_mat = Eigen::MatrixXd::Zero(dim,dim);
    cov.GetCovarianceMatrix(blocks_list, cov_mat.data());
    return cov_mat;
...
{
    const size_t num_cams = initial_cameras.size();
    if (num_cams==0) throw std::invalid_argument("No camera intrinsics provided");

    ScheimpflugBundleBlocks blocks = initialize_blocks_scheimpflug(
        initial_cameras, init_g_T_r, init_c_T_r, init_b_T_t);

    ceres::Problem p; build_problem_scheimpflug(observations, opts, blocks, p);
