...
struct ExtrinsicResidual final {
  ...
  template <typename T>
  bool operator()(const T* c_q_r, const T* c_t_r, const T* r_q_t,
                  const T* r_t_t, const T* intrinsics, T* residuals) const {
    const auto [c_R_t, c_t_t] =
        get_camera_se3_target(quat_array_to_rotmat(c_q_r),
                              array_to_translation(c_t_r),
                              quat_array_to_rotmat(r_q_t),
                              array_to_translation(r_t_t));
