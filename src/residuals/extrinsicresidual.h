/** @brief Residual for extrinsic optimization */
...
struct ExtrinsicResidual final {
    ...
    ExtrinsicResidual(PlanarView&& v, const CameraT& cam)
        : view(std::move(v)), cam_(cam) {}

    template <typename T>
    bool operator()(const T* c_q_r, const T* c_t_r,
                    const T* r_q_t, const T* r_t_t, T* residuals) const {
    ...
    static auto* create(const PlanarView& view, const CameraT cam) {
        auto* functor = new ExtrinsicResidual(view, cam);
        auto* cost = new ceres::AutoDiffCostFunction<
            ExtrinsicResidual, ceres::DYNAMIC, 4, 3, 4, 3>(
