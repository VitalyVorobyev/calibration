...
struct JointBlocks final : public ProblemParamBlocks {
    ...
    void populate_result(JointOptimizationResult<CameraT>& result) const {
...
static ceres::Problem build_problem(
    ...
    const std::vector<CameraT>& cameras,
    const JointIntrExtrOptions& options,
    ...
    for (size_t view_idx = 0; view_idx < views.size(); ++view_idx) {
        ...
        for (size_t cam_idx = 0; cam_idx < cameras.size(); ++cam_idx) {
            ...
            p.AddResidualBlock(
                JointIntrExtrResidual<CameraT>::create(view),
                loss,
...
template JointOptimizationResult<Camera<BrownConradyd>> optimize_joint_intrinsics_extrinsics(
    ...
    const std::vector<Eigen::Affine3d>&,
    const JointIntrExtrOptions&);

// Scheimpflug Camera cannot be optimized because of the system is underconstrained
#if 0
