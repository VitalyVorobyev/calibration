...
TEST(ScheimpflugHandEye, ReprojectionRefinement) {
    ...
    for (int i=0;i<8;++i){
        ...
        b_T_g.push_back(btg);
        Eigen::Affine3d c_T_t = compute_camera_T_target(b_T_t, g_T_r, Eigen::Affine3d::Identity(), btg);
    ...
    Eigen::Affine3d init_g = g_T_r; init_g.translation() += Eigen::Vector3d(0.01,-0.01,0.02);
    ReprojRefineOptions opts; opts.refine_intrinsics=true; opts.refine_distortion=false; opts.use_distortion=false; opts.verbose=false;
    auto res = refine_hand_eye_reprojection_scheimpflug(b_T_g, obs, init_intr, init_g, opts);
...
TEST(ScheimpflugBundle, SingleCamera) {
    ...
    Eigen::Affine3d init_g = g_T_r; init_g.translation() += Eigen::Vector3d(0.01,-0.01,0.02);
    BundleOptions opts; opts.optimize_intrinsics=true; opts.optimize_target_pose=false; opts.optimize_hand_eye=true; opts.optimize_extrinsics=false; opts.verbose=false;
    auto res = optimize_bundle_scheimpflug(observations, cams, init_g, {Eigen::Affine3d::Identity()}, b_T_t, opts);
    EXPECT_LT((res.g_T_r.translation()-g_T_r.translation()).norm(),1e-3);
    Eigen::AngleAxisd diff(res.g_T_r.linear()*g_T_r.linear().transpose());
    EXPECT_LT(diff.angle(),1e-3);
